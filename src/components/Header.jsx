...
const Header = memo(({ toggleTheme, currentTheme, onHamburgerClick }) => {
  ...
  return (
    ...
    >
      ...
      <Link
        to="/"
        className="text-2xl sm:text-3xl font-extrabold text-primary tracking-wide hover:opacity-80 transition"
      >
        Arjun Meghwal
      </Link>
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              key={link.to}
              to={link.to}
              className={`px - 3 py - 1.5 rounded - md text - base font - medium transition - all duration - 200
                ${
  isActive
